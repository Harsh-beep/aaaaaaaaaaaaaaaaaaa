...
const isMuted = (chatId) => {
		...
		cron.schedule('0 0 * * *', () => {
            const reset = []
            amvlimit = reset
            console.log('Hang tight, it\'s time to reset amv usage limits...')
            fs.writeFileSync('./settings/amvlimit.json', JSON.stringify(amvlimit))
            console.log('Success!')
        }, {
            scheduled: true,
            timezone: 'Asia/Kolkata'
...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        const groupId = isGroupMsg ? chat.groupMetadata.id : 'https://chat.whatsapp.com/KGzcSuJik0TI3lk2Eh58G0'
		...
		const StickerMetadatacrop = { author : 'BOT', pack: 'Akeno', keepScale: false }

        // [IDENTIFY]
        const ownerNumber = ["919118881949@c.us","15632317376@c.us"]
		const ownerNumberr = "15632317376@c.us"
	...
	/*if (chats.includes('akenooo chan')) { /*Y'ALL CAN CUSTOM THIS WHATEVER YOU WANT!
	...
	const addAfk = (userId, time) => {
		...
		const bayarLimitt = (serial, amount) => {
            let position = false
            Object.keys(amvlimit).forEach((i) => {
                if (amvlimit[i].id === serial) {
                    position = i
                }
            })
            if (position !== false) {
                amvlimit[position].amvlimit -= amount;
                fs.writeFileSync('./settings/amvlimit.json', JSON.stringify(amvlimit))
            }
        }
	...
	const isAfkOn = getAfk(sender.id)
		...
		function isamvLimit(userId){
                    if (isOwnerBot) {return false;}
                    let found = false;
                    for (let i of amvlimit){
                        if(i.id === userId){
                            let amvlimits = i.amvlimit;
                            if (amvlimits >= amvlimitCount) {
                                found = true;
                                return true;
                            }else{
                                amvlimit
                    ...
                    if (found === false){
                        let obj = {id: `${userId}`, amvlimit:0};
                        amvlimit.push(obj);
                        fs.writeFileSync('./settings/amvlimit.json',JSON.stringify(amvlimit));
                        return false;
                    }  
                }
		function amvlimitAdd(userId) {
                    if (isOwnerBot) {return;}
                    var found = false;
                    Object.keys(amvlimit).forEach((i) => {
                        if(amvlimit[i].id == userId){
                            found = i
                        }
                    })
                    if (found !== false) {
                        amvlimit[found].amvlimit += 1;
                        fs.writeFileSync('./settings/amvlimit.json',JSON.stringify(amvlimit));
                    }
                }
		...
		const menupict = [
			"https://c4.wallpaperflare.com/wallpaper/273/77/151/anime-girls-highschool-dxd-himejima-akeno-wallpaper-preview.jpg",
			"https://wallpapercave.com/wp/wp7937145.jpg",
			"https://i.imgur.com/vBsE62j.jpg",
			"https://wallpapercave.com/wp/wp2098969.jpg",
			]
		const slapp = [
	...
	// Anti spam
		...
		aruga.reply(from, mess.wait, id)
		aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/anime/storyanime?apikey=1QM9kTIM1WUkJ57`, 'anime.mp4', '', id)
		limitAdd(serial)
		break
         ...
         case prefix+'kbbi00000000000000000000':
			...
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				...
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                    ...
                    aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/photooxy/wolfmetal?text=${srhdaahh}&apikey=1QM9kTIM1WUkJ57 `, `${srhdaahh}.jpg`, `here it is...`, id).catch(async () => { await aruga.reply(from, `Error!!\nIf used emoji then pls don't..`, id) })
					limitAdd(serial)
                    break
					case prefix+'flame':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                    ...
                    aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/photooxy/flaming?text=${srhddaahh}&apikey=1QM9kTIM1WUkJ57 `, `${srhddaahh}.jpg`, `here it is...`, id).catch(async () => { await aruga.reply(from, `Error!!\nIf used emoji then pls don't..`, id) })
					limitAdd(serial)
                    break
				...
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                    ...
                    aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/photooxy/romantic?text=${kawai}&apikey=1QM9kTIM1WUkJ57`, `${kawai}.jpg`, `here it is...`, id).catch(async () => { await aruga.reply(from, `Error!!\nIf used emoji then pls don't..`, id) })
					limitAdd(serial)
                    break*/
...
if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
            ...
            break
			   ...
			   case prefix+'neko3':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				   if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
               aruga.reply(from, mess.wait, id);
			   aruga.sendFileFromUrl(from,`https://dapuhy-api.herokuapp.com/api/sfw/sfwneko?apikey=1QM9kTIM1WUkJ57`, ' ' ,'', id)
				limitAdd(serial)
               break
			   case prefix+'foxgirl2':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				   if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
               aruga.reply(from, mess.wait, id);
			   aruga.sendFileFromUrl(from,`https://dapuhy-api.herokuapp.com/api/sfw/sfwfoxgirl?apikey=1QM9kTIM1WUkJ57`, ' ' ,'', id)
				limitAdd(serial)
               break
			   case prefix+'kemonomimi':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				   if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
               aruga.reply(from, mess.wait, id);
			   aruga.sendFileFromUrl(from,`https://dapuhy-api.herokuapp.com/api/sfw/sfwkemonomimi?apikey=1QM9kTIM1WUkJ57`, ' ' ,'', id)
				limitAdd(serial)
               break
			   case prefix+'holo':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				   if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
               aruga.reply(from, mess.wait, id);
			   aruga.sendFileFromUrl(from,`https://dapuhy-api.herokuapp.com/api/sfw/sfwholo?apikey=1QM9kTIM1WUkJ57`, ' ' ,'', id)
				limitAdd(serial)
               break
			   case prefix+'animeavatar2':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				   if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
               aruga.reply(from, mess.wait, id);
			   aruga.sendFileFromUrl(from,`https://dapuhy-api.herokuapp.com/api/sfw/sfwwaifu?apikey=1QM9kTIM1WUkJ57`, ' ' ,'', id)
				limitAdd(serial)
               break
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'This command can only be used when bot becoms admin', id)
        ...
        //Media 
            case prefix+'ytmp32':
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                if (args.length == 0) return aruga.reply(from, `Download a youtube video as mp3\nType: ${prefix}ytmp3 [link_yt]`, id)
                aruga.reply(from, mess.wait, id)
				axios.get(`https://api.zeks.me/api/ytmp3?apikey=dc4x8bqxoKDYOGMAwlnOyOnvidk&url=${args[0]} `)
                .then(async(res) => {
				await aruga.sendFileFromUrl(from, res.data.result.thumbnail, '', `「 *YOUTUBE MP3* 」\n\n*Title:* ${res.data.result.title}\n\n\n*_Waitt, lemme send that audio_*`, id)
				await aruga.sendFileFromUrl(from, res.data.result.url_audio, 'song.mp3', id)
				limitAdd(serial)
			})
			.catch(err => {
				aruga.reply(from, 'error', id)
			...
			case prefix+'ytmp3':
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
				if (args.length == 0) return await aruga.reply(from, `This command needs something to work, for it to work insert a Youtube Music Video Title`, id)
				try {
					limitAdd(serial)
					await aruga.reply(from, mess.wait, id)
					const ytres = await ytsearch(`${body.slice(7)}`)
					await aruga.sendYoutubeLink(from, `${ytres.all[0].url}`, '\n' + `*Title >* ${ytres.all[0].title}\n*Link >* https://youtu.be/${ytres.all[0].videoId}\n*Duration >* ${ytres.all[0].timestamp} minutes\n*Views >* ${ytres.all[0].views}\n*Channel >* ${ytres.all[0].author.url}`)
					await youtubedl(`https://youtu.be/${ytres.all[0].videoId}`, { noWarnings: true, noCallHome: true, noCheckCertificate: true, preferFreeFormats: true, youtubeSkipDashManifest: true, referer: `https://youtu.be/${ytres.all[0].videoId}`, x: true, audioFormat: 'mp3', o: `./lib/media/audio/${sender.id.replace('@c.us', '')}${lvpc}.mp3` }).then(async () => { await aruga.sendPtt(from, `./lib/media/audio/${sender.id.replace('@c.us', '')}${lvpc}.mp3`, id) })
					.then(async () => { await fs.unlinkSync(`./lib/media/audio/${sender.id.replace('@c.us', '')}${lvpc}.mp3`) })
				} catch (err) {
					await aruga.reply(from, `Error!!.`, id)
					if (fs.existsSync(`./lib/media/audio/${sender.id.replace('@c.us', '')}${lvpc}.mp3`)) { await fs.unlinkSync(`./lib/media/audio/${sender.id.replace('@c.us', '')}${lvpc}.mp3`) }
					console.log(color('[PLAY]', 'crimson'), color(`→ I got errors in the command .`, 'gold'))
				}
				break
			...
			if (isMedia && isImage || isQuotedImage) {
				...
				if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/canvas/delete?img=${iiinimagee}&apikey=1QM9kTIM1WUkJ57`, 'deltrash.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/rip?img=${iiinnnnimagee}&apikey=1QM9kTIM1WUkJ57`, 'rip.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						const mediaData = await decryptMedia(encrypt, uaOverride)
						const inimage = await uploadImages(mediaData, `${sender.id}_img`)
						await aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/canvas/continued?img==${inimage}&apikey=1QM9kTIM1WUkJ57`, 'tobe.jpg', '', id)
						limitAdd(serial)
						.catch(() => {
							aruga.reply(from, 'Lagi error', id)
						})
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/gay?img=${iimaagggeee}&apikey=1QM9kTIM1WUkJ57`, 'gya.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/crush?img=${iimmmaaagggeee}&apikey=1QM9kTIM1WUkJ57`, 'bnw.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/meth?img=${iiimmmaaagggeee}&apikey=1QM9kTIM1WUkJ57`, 'bnw.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/hitler?img=${iiiimmmmmaaagggeee}&apikey=1QM9kTIM1WUkJ57`, 'bnw.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, ` https://dapuhy-api.herokuapp.com/api/canvas/blur?img=${iiiimmmaaagggeee}&level=7&apikey=1QM9kTIM1WUkJ57`, 'bnw.jpg', '', id)
						limitAdd(serial)
					...
					if (isMedia || isImage || isQuotedImage) {
						...
						await aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/canvas/beautiful?img1=${iiiiiimmmaaagggeee}&img2=https://i.ibb.co/Zxg1HB6/akaza2.jpg&apikey=1QM9kTIM1WUkJ57`, 'bnw.jpg', '', id)
						limitAdd(serial)
					} else {
						aruga.reply(from, 'Message format is wrong..\n Please send or reply to a image..', id)
					}
...
aruga.reply(from, mess.wait, id)
try {
	const javhd = await axios.get(`https://dapuhy-api.herokuapp.com/api/search/javhdporn?query=${javv}&apikey=1QM9kTIM1WUkJ57`)
...
aruga.reply(from, mess.wait, id)
try {
	const javhd1 = await axios.get(`https://dapuhy-api.herokuapp.com/api/search/javhdd?query=${jaavv}&apikey=1QM9kTIM1WUkJ57`)
...
🌐️ *URL*: ${url}`
		...
		if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
		aruga.reply(from, mess.wait, id)
		axios.get(`https://dapuhy-api.herokuapp.com/api/randomimage/couple?apikey=1QM9kTIM1WUkJ57`)
		.then(async(res) => {
	...
	aruga.reply(from, mess.wait, id)
	try {
		const kusoon = await axios.get(`https://dapuhy-api.herokuapp.com/api/anime/mal-search?query=${carianiim}&apikey=1QM9kTIM1WUkJ57 `)
	...
	aruga.reply(from, mess.wait, id)
	try {
		const kuusoon = await axios.get(`https://dapuhy-api.herokuapp.com/api/manga/animeplanet?query=${carrianiim}&apikey=1QM9kTIM1WUkJ57 `)
	   ...
	   //if (!isPrem) return aruga.reply(from, mess.prem, id)
	    ...
	    break
			...
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                ...
                aruga.reply(from, mess.wait, id)
				const searchig = body.slice(10)
				axios.get(`https://docs-jojo.herokuapp.com/api/stalk?username=${searchig}`)
				.then(async(res) => {
					const picture = res.data.profile_pic
					const Biodata = res.data.biography
					const jumlahfol = res.data.followers
					const jumlahfollow = res.data.following
					//const jumlahpost = res.data.data.timeline
					const Namaig = res.data.name
					const UsernameIG = res.data.username
					const cption = `• *Username:* ${UsernameIG}\n• *Name:* ${Namaig}\n• *Followers:* ${jumlahfol}\n• *Following:* ${jumlahfollow}\n\n• *Bio:* ${Biodata}`
					await aruga.sendFileFromUrl(from, res.data.profile_pic, '', cption, id)
					limitAdd(serial)
		...
		if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
            ...
            .then(async (res) => {
                await aruga.reply(from, `Lyrics : ${body.slice(7)}\n\n${res.lirik}`, id)
				limitAdd(serial)
            })
            break
			case prefix+'lyrics':
		...
		if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
            if (args.length == 0) return aruga.reply(from, `To search for the lyrics of a song\bType: ${prefix}lyrics song title`, id)
            const llyricc = (body.slice(8))
				axios.get(`https://hardianto-chan.herokuapp.com/api/info/lirik?query=${llyricc}&apikey=hardianto`)
				.then(async (res) => {
                aruga.reply(from, res.data.thumb, 'thumb.jpg', `${res.data.lirik}`, id)
				limitAdd(serial)
            })
            break
	...
	break
            case prefix+'twitter0000':
                if (args.length == 0) return aruga.reply(from, `Send orders ${prefix}twitter [linktwitter]`, id)
                aruga.reply(from, mess.wait, id)
...
console.log(err)
		...
		aruga.reply(from, mess.wait, id)
			...
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
                ...
                aruga.reply(from, mess.wait, id)
				axios.get(`https://zenzapi.xyz/api/downloader/ytmp4?url=${args[0]}&apikey=4fedf539f761 `)
                .then(async(res) => {
				//if (Number(res.data.result.size.split(' MB')[0]) >= 25.0) return aruga.sendFileFromUrl(from, res.data.result.thumb, ``, `「 *YOUTUBE MP4* 」\n\nTitle: ${res.data.result.title}\nSize: ${res.data.result.size}\n\n*FILE SIZE MORE THAN 25MB*\n*PLEASE ASK FOR A SHORTER VIDEO*`, id)
				await aruga.sendFileFromUrl(from, res.data.result.thumb, '', `「 *YOUTUBE MP4* 」\n\n*Title:* ${res.data.result.title}\n*_Waitt, lemme send that video_*`, id)
				await aruga.sendFileFromUrl(from, res.data.result.url, '',`${res.data.result.title}` , id)
				limitAdd(serial)
		...
		case prefix+'wallpaper':
            ...
            case prefix+'play000000000000000000'://silahkan kalian custom sendiri jika ada yang ingin diubah
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
           ...
           axios.get(`http://docs-jojo.herokuapp.com/api/yt-search?q=${body.slice(6)}`)
			...
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
           if (args.length == 0) return aruga.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: ${prefix}play judul lagu`, id)
           axios.get(`https://api.zeks.me/api/ytplaymp3/2?apikey=dc4x8bqxoKDYOGMAwlnOyOnvidk&q=${body.slice(7)}`)
            .then(async (res) => {
				if (Number(res.data.result.size.split(' MB')[0]) >= 25.0) return aruga.sendFileFromUrl(from, res.data.result.thumb, ``, `「 *PLAY* 」\n\nName: ${res.data.result.title}\nDuration: ${res.data.result.duration} mins\nSize: ${res.data.result.size}\nSource: ${res.data.result.source}\n\n*FILE SIZE MORE THAN 25MB*\n*PLEASE CHANGE THE KEYWORD*`, id)
                 await aruga.sendFileFromUrl(from, res.data.result.thumb, ``, `「 *PLAY* 」\n\nName: ${res.data.result.title}\nDuration: ${res.data.result.duration} mins\nSize: ${res.data.result.size}\nSource: ${res.data.result.source}\n\n*_Wait, sending the audio_*`, id)
			...
			if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
           if (args.length == 0) return aruga.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: ${prefix}play judul lagu`, id)
           axios.get(`https://api.zeks.me/api/ytplaymp3?apikey=dc4x8bqxoKDYOGMAwlnOyOnvidk&q=${body.slice(7)}`)
            .then(async (res) => {
				if (Number(res.data.result.size.split(' MB')[0]) >= 25.0) return aruga.sendFileFromUrl(from, res.data.result.thumb, ``, `「 *PLAY* 」\n\nName: ${res.data.result.title}\nDuration: ${res.data.result.duration} mins\nSize: ${res.data.result.size}\nSource: ${res.data.result.source}\n\n*FILE SIZE MORE THAN 25MB*\n*PLEASE CHANGE THE KEYWORD*`, id)
                 await aruga.sendFileFromUrl(from, res.data.result.thumbnail, ``, `「 *PLAY* 」\n\nName: ${res.data.result.title}\nDuration: ${res.data.result.duration} mins\nSize: ${res.data.result.size}\nSource: ${res.data.result.source}\n\n*_Wait, sending the audio_*`, id)
				await aruga.sendFileFromUrl(from, res.data.result.url_audio, ' ', ' ', id)
				limitAdd(serial)
                        })
                        break
						case prefix+'audio':
	...
	case prefix+'remove':
		...
		aruga.sendRawWebpAsSticker(from, `https://dapuhy-api.herokuapp.com/api/maker/attp?text=${txttx}&apikey=1QM9kTIM1WUkJ57`, {author: authorr, pack: pack }, id)
		limitAdd(serial)
		break
		case prefix+'attp':
		if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
		...
		const jumlahimage1 = q.split('|')[1]
		if (jumlahimage1 > 10) return aruga.reply(from, 'Maximum 10', id)
		const imaaaggg = googleImage(`${searchimage1}`).then(results => {
			console.log(results)
			for (let i = 0; i < jumlahimage1; i++) {
				aruga.sendFileFromUrl(from, results[i].url, '', '', id)
			}
		 })
		limitAdd(serial)
		break
	...
	case prefix+'odelprem':
		...
		if (isMedia || isImage || isQuotedImage) {
			...
			aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/canvas/trash?img=${uploadImagex}&apikey=1QM9kTIM1WUkJ57`, '', '', id)
			limitAdd(serial)
		} else {
			aruga.reply(from, 'Message format is wrong', id)
		}
		...
		case prefix+'randomhentai':
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwneko?apikey=1QM9kTIM1WUkJ57`, ' ','Nyaa~~', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwavatar?apikey=1QM9kTIM1WUkJ57`, ' ','Nyaa~~', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwyuri?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
			case prefix+'tits':
            if (!isGroupMsg) return aruga.reply(from, 'This command can only be used in groups!', id)
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwtits?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwgirlsolo?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwpussyart?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwkemonomimi?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwkitsune?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwketa?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwfemdom?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwerokitsune?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfweroyuri?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwcumarts?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
			case prefix+'nsfwbj2':
            if (!isGroupMsg) return aruga.reply(from, 'This command can only be used in groups!', id)
            ...
            //if (!isPrem) return aruga.reply(from, mess.prem, id)
				if (!isAnime) return await aruga.reply(from, '*Turn on Anime mode please...*', id)
				   if (!isNsfw) return await aruga.reply(from, '*Turn on NSFW mode please...*', id)
            aruga.sendText(from, mess.wait);
            aruga.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/nsfw/nsfwblowjob?apikey=1QM9kTIM1WUkJ57`, ' ','', id)
			limitAdd(serial)
            break
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us','')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
                ...
                case prefix+'santet': //work
					...
					if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
						...
						aruga.sendFileFromUrl(from, res.data.message, 'Dog.jpeg', 'Doggo sparkles', id)
						limitAdd(serial)})
                        break
					case prefix+'cat':  
					if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)					
		...
		aruga.sendImageAsSticker(from, `https://dapuhy-api.herokuapp.com/api/maker/ttp?text=${txxttxx}&apikey=1QM9kTIM1WUkJ57`, {author: authorr, pack: pack }, id)
		limitAdd(serial)
		break
		case prefix+'attp2':
		if (isLimit(serial)) return aruga.reply(from, `_Hello ${pushname} Your request limit has reached the limit. Please come back tomorrow!_\n*(Daily limit reset at 12:00am IST)*`, id)
        ...
        case prefix+'howmuch':
            ...
            const yaelah = chat.groupMetadata.participants.length
                ...
                break
        case prefix+'ownergc':
            await aruga.sendLinkWithAutoPreview(from, `Owner Group : https://chat.whatsapp.com/JUg5EYMB5sYBJA305WYSxl`,'','', id)
            break
		case prefix+'nsfwgc':
            await aruga.sendLinkWithAutoPreview(from, `Owner NSFW Group : https://chat.whatsapp.com/DeZYQf5sX670jAJSHZ8jlW`,'','', id)
            break
        }
